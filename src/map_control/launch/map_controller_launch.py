...
def generate_launch_description():
    ...
    Features:
    - Restored FORZA curvature-based speed limiting (kappa)
    - Restored FORZA heading-based speed adjustment (psi)
    - Full CSV format support (x, y, v, d, s, kappa, psi, ax)
    - Simulation: Odom GT pose (not TF)
    - Multi-lane support
    ...
    map_controller_node = Node(
        package='map_control',
        executable='map_controller',  # 🔧 Using OJG test version with FORZA features
        name='map_controller',
        output='screen',
        parameters=[config],
    )
