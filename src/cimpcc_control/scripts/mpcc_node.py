...
class MPCKinematicNode(Node):
    def __init__(self):
        ...
        # CiMPCC parameters
        self.declare_parameter('mpc_w_curvature', 40.0)
        self.declare_parameter('curvature_alpha', 3.0)
        ...
        self.param = {
            ...
            'mpc_w_curvature': self.get_parameter('mpc_w_curvature').value,
            'curvature_alpha': self.get_parameter('curvature_alpha').value,
    ...
    def initialize_MPC(self):
        ...
        # CiMPCC: Set curvature lookup table
        self.mpc.set_curvature_lut(self.curvature_lut)
    ...
    def compute_curvature_from_derivatives(self, derivatives):
        """
        Compute curvature from derivative data (dx, dy).
        Curvature κ = ||d(dx)/ds * dy - dx * d(dy)/ds|| / (dx^2 + dy^2)^(3/2)
        For normalized derivatives (unit tangent vectors): κ ≈ ||dθ/ds||
        """
        curvatures = []
        for i in range(1, len(derivatives) - 1):
            # Current derivatives
            dx_curr = derivatives[i, 0]
            dy_curr = derivatives[i, 1]

            # Derivative changes (approximating second derivative)
            ddx = derivatives[i+1, 0] - derivatives[i-1, 0]
            ddy = derivatives[i+1, 1] - derivatives[i-1, 1]

            # Curvature formula
            numerator = np.abs(dx_curr * ddy - ddx * dy_curr)
            denominator = (dx_curr**2 + dy_curr**2)**(3/2)
        ...
        # Handle boundaries (use nearest value)
        curvatures = [curvatures[0]] + curvatures + [curvatures[-1]]

        return np.array(curvatures)

    def smooth_curvature(self, curvatures, window=5):
        """
        Smooth curvature using Moving Average Filter (MAF).
        Paper Eq. (7)
        """
        if window < 1:
            return curvatures
        ...
        smoothed = np.convolve(curvatures, np.ones(window)/window, mode='same')

        return smoothed

    def normalize_curvature(self, curvatures):
        """
        Normalize curvature to [0, 1] range.
        Paper Eq. (8): K_n = (K - K_min) / (K_max - K_min)
        """
        K_min = np.min(curvatures)
        K_max = np.max(curvatures)

        if K_max - K_min > 1e-6:
            K_n = (curvatures - K_min) / (K_max - K_min)
        else:
            K_n = np.zeros_like(curvatures)
    ...
    def preprocess_track_data(self):
        ...
        # CiMPCC: Compute, smooth, and normalize curvature from derivatives
        curvatures = self.compute_curvature_from_derivatives(center_derivative_data)
        curvature_window = self.get_parameter('curvature_window').value if self.has_parameter('curvature_window') else 5
        smoothed_curvatures = self.smooth_curvature(curvatures, window=curvature_window)
        self.normalized_curvatures = self.normalize_curvature(smoothed_curvatures)

        # Create CasADi interpolation for normalized curvature
        from casadi import interpolant
        self.curvature_lut = interpolant('lut_curvature', 'bspline', [self.element_arc_lengths], self.normalized_curvatures)

        self.get_logger().info(f"Curvature stats: min={np.min(curvatures):.4f}, max={np.max(curvatures):.4f}, mean={np.mean(curvatures):.4f}")
