...
class MPC:
    def __init__(self):
        ...
        # CiMPCC: Curvature-based velocity optimization
        self.curvature_lut = None
        self.curvature_alpha = 3.0  # Mapping sensitivity (will be set from params)
        self.v_upper = None  # Aggressive velocity bounds
        self.v_lower = None  # Safe velocity bounds
    ...
    def init_system_model(self):
        ...
        self.P = MX.sym('P', self.n_states + 2 * self.N + self.N * self.n_controls)  # + 4 * self.N_OBST (DISABLED)
        # parameters (which include the initial state, path boundaries, and reference controls along the predicted trajectory)
        ...
        # Note: R[2,2] for p is now 0 because we use linear maximization term instead of quadratic tracking
    ...
    def set_curvature_lut(self, curvature_lut):
        """Set the curvature lookup table for CiMPCC"""
        self.curvature_lut = curvature_lut

        # Set velocity bounds for curvature-based optimization
        # Paper: v_bar (aggressive) and v_underbar (safe)
        # Only apply to vehicle velocity v, NOT to progress velocity p
        v_max = self.param['v_max']
        self.v_upper = v_max  # Aggressive velocity (straight sections)
        self.v_lower = v_max * 0.5  # Safe velocity (high curvature corners)

        # Get curvature alpha from params if available
        if 'curvature_alpha' in self.param:
            self.curvature_alpha = self.param['curvature_alpha']

    def curvature_velocity_mapping(self, K_n):
        """
        Map normalized curvature to velocity coefficient.
        Paper Eq. (9): g(K_n) = e^(-α·(K_n)^2)
        Returns β ∈ [e^(-α), 1]
        """
        from casadi import exp
        beta = exp(-self.curvature_alpha * K_n**2)
        return beta
    ...
    def compute_optimization_cost(self):
        ...
        for k in range(self.N):
            ...
            con_start_idx = self.n_states + 2 * self.N + self.n_controls * k  # + 4 * self.N_OBST (DISABLED)
            con_end_idx = self.n_states + 2 * self.N + self.n_controls * (k + 1)  # + 4 * self.N_OBST (DISABLED)

            # Path following cost (contouring + lag error)
            self.obj = self.obj + mtimes(mtimes(error.T, self.Q), error)

            # Control effort cost (velocity, steering, progress tracking)
            self.obj = self.obj + mtimes(mtimes((con - self.P[con_start_idx:con_end_idx]).T,self.R), (con - self.P[con_start_idx:con_end_idx]))

            # Progress maximization (C++ MPCC approach: linear term)
            # Negative because we want to MAXIMIZE p (minimize -p)
            self.obj = self.obj - self.param['mpc_w_p'] * con[2]  # con[2] is p (progress velocity)

            # CiMPCC: Curvature-based velocity optimization (Paper Eq. 10)
            if self.curvature_lut is not None and self.param.get('mpc_w_curvature', 0) > 0:
                # Get normalized curvature at current progress position
                K_n = self.curvature_lut(st_next[3])  # st_next[3] is s (progress)

                # Compute velocity mapping coefficient β = g(K_n)
                beta = self.curvature_velocity_mapping(K_n)

                # Vehicle velocity only (NOT progress velocity)
                v_k = con[0]  # Vehicle velocity

                # Curvature-based velocity cost
                # J_Ci = (1-β)||v - v_lower||² + β||v - v_upper||²
                # High curvature (corner): β→0, penalty pushes v → v_lower (slow)
                # Low curvature (straight): β→1, penalty pushes v → v_upper (fast)
                w_curv = self.param['mpc_w_curvature']
                self.obj = self.obj + w_curv * (1 - beta) * (v_k - self.v_lower)**2
                self.obj = self.obj + w_curv * beta * (v_k - self.v_upper)**2
            ...
            # OBSTACLE AVOIDANCE DISABLED: Causing numerical instability
            # TODO: Implement soft constraint or fix numerical issues
            # # Obstacle avoidance constraints
            # # For each detected obstacle, add circular constraint
            # # Constraint: (x - obs_x)^2 + (y - obs_y)^2 >= obs_radius^2
            # # Parameters: [obs1_x, obs1_y, obs1_r, obs1_active, obs2_x, ...]
            # for obs_idx in range(self.N_OBST):
            #     obs_base_idx = self.n_states + 2 * self.N + obs_idx * 4
            #     obs_x = self.P[obs_base_idx]
            #     obs_y = self.P[obs_base_idx + 1]
            #     obs_r = self.P[obs_base_idx + 2]
            #     # obs_active = self.P[obs_base_idx + 3]  # Not used for now

            #     obs_dist_sq = (st_next[0] - obs_x)**2 + (st_next[1] - obs_y)**2
            #     self.g = vertcat(self.g, obs_dist_sq - obs_r**2)
    ...
    def init_constraints(self):
        ...
        # Constraints: initial + dynamics + path bounds (obstacle avoidance DISABLED)
        n_constraints = self.n_states * (self.N + 1) + self.N  # + self.N * self.N_OBST
    ...
    def get_path_constraints_points(self, prev_soln):
        ...
        for k in range(1, self.N + 1):
            right_points[k - 1, 0] = float(self.right_lut_x(prev_soln[k, 3]))
            right_points[k - 1, 1] = float(self.right_lut_y(prev_soln[k, 3]))
            left_points[k - 1, 0] = float(self.left_lut_x(prev_soln[k, 3]))
            left_points[k - 1, 1] = float(self.left_lut_y(prev_soln[k, 3]))
    ...
    def construct_warm_start_soln(self, initial_state):
        ...
        for k in range(1, self.N + 1):
            ...
            self.X0[k, 0] = float(x_next)
            self.X0[k, 1] = float(y_next)
            self.X0[k, 2] = float(psi_next)
            self.X0[k, 3] = float(s_next)
    ...
    def solve(self, initial_state):
        p = np.zeros(self.n_states + 2 * self.N + self.N * self.n_controls)  # + 4 * self.N_OBST (DISABLED)
        ...
        for k in range(self.N):
            ...
            theta_ref = 0
            p[self.n_states + 2 * self.N + self.n_controls * k:self.n_states + 2 * self.N + self.n_controls * (k + 1)] = [v_ref, theta_ref, p_ref]
    ...
    def print_solver_statistics(self):
        """Print solver timing statistics"""
        if len(self.solve_times) == 0:
            print("[SOLVER STATS] No solve times recorded")
            return

        times_ms = np.array(self.solve_times) * 1000

        print("\n" + "="*60)
        print("SOLVER STATISTICS - MUMPS")
        print("="*60)
        print(f"Total iterations:  {len(times_ms)}")
        ...
        print(f"Total time:        {np.sum(times_ms)/1000:.2f} s")
        print("="*60 + "\n")
    ...
    def publish_boundary_markers(self, right_points, left_points):
        ...
        for i in range(combined_points.shape[0]):
            ...
            path_marker.color = ColorRGBA(r=0.0, g=0.0, b=1.0, a=0.8)
